...
public class User {
    ...
    private String validateEmail(String email) {
        if (email == null || !email.matches("^[A-Za-z0-9+_.-]+@(.+)$")) {
            throw new InvalidDomainDataException("Email", "Invalid email format");
        }
        return email;
    }
    ...
    private String validateName(String name) {
        if (name == null || name.isBlank()) {
            throw new InvalidDomainDataException("Name", "Name cannot be empty");
        }
        return name.trim();
    }
    ...
    private String validatePassword(String password) {
        if (password == null || password.length() < 6) {
            throw new InvalidDomainDataException("Password", "Password must have at least 6 characters");
        }
        return password;
    }
